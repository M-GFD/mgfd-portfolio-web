'use client';

import { useLanguage } from '@/contexts/LanguageContext';
import { cn } from '@/lib/utils';

type FooterProps = {
  embedded?: boolean;
  showCta?: boolean;
};

export default function Footer({ embedded = false, showCta = false }: FooterProps) {
  const { t } = useLanguage();

  const ctaLink = showCta ? (
    <a
      href="mailto:mgfd.design@gmail.com"
      className="inline-flex shrink-0 items-center justify-center rounded-lg bg-white px-4 py-1.5 text-xs font-medium text-black shadow-sm shadow-black/40 transition-colors hover:bg-neutral-200 sm:px-5 sm:py-2 sm:text-sm"
    >
      {t('projects.cta')}
    </a>
  ) : null;

  return (
    <footer
      id="contact"
      className={cn(
        'border-none bg-zinc-950/86 pb-[env(safe-area-inset-bottom)] shadow-[0_-8px_30px_-12px_rgba(0,0,0,0.45)] backdrop-blur-[10px] transition-[backdrop-filter] duration-300 ease-out hover:backdrop-blur-[12px]',
        embedded ? 'mt-4 shrink-0 sm:mt-5' : 'mt-6 sm:mt-8 md:mt-10',
      )}
    >
      <div
        className={cn(
          'mx-auto max-w-7xl px-4 sm:px-6',
          embedded ? 'py-3 sm:py-3.5' : 'py-8 sm:py-10 md:py-12',
        )}
      >
        {embedded && showCta ? (
          <div className="flex flex-col gap-2 sm:gap-2.5">
            <div className="flex w-full items-center justify-between gap-3">
              <div className="flex min-w-0 items-center gap-3 sm:gap-4">
                <img
                  src="images/_mgfd_logo.svg"
                  alt="Mateo G. Fontana Dalmasso (MGFD) — portfolio web"
                  className="h-7 w-auto shrink-0 invert brightness-0 contrast-200 sm:h-8"
                />
                <div className="flex shrink-0 justify-center gap-3.5 sm:gap-4">
                  <a
                    href="https://instagram.com/xmgfd"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-neutral-400 transition-colors hover:text-white"
                    aria-label="Instagram"
                  >
                    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                    </svg>
                  </a>
                  <a
                    href="https://linkedin.com/in/mateofontanad"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-neutral-400 transition-colors hover:text-white"
                    aria-label="LinkedIn"
                  >
                    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
                    </svg>
                  </a>
                  <a
                    href="https://twitter.com/mgfd_"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-neutral-400 transition-colors hover:text-white"
                    aria-label="X"
                  >
                    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                    </svg>
                  </a>
                </div>
              </div>
              {ctaLink}
            </div>
            <div className="text-center text-[0.6rem] leading-tight tracking-tight text-neutral-400 min-[400px]:text-[0.65rem] sm:text-xs sm:tracking-normal">
              {t('footer.copyright')}
            </div>
          </div>
        ) : (
          <div
            className={cn(
              'flex flex-col items-center justify-between md:flex-row',
              embedded ? 'gap-4 sm:gap-5' : 'gap-6 sm:gap-8',
            )}
          >
            <div className="flex flex-col text-center md:text-left">
              <div className={cn(embedded ? 'mb-2 sm:mb-3' : 'mb-4')}>
                <img
                  src="images/_mgfd_logo.svg"
                  alt="Mateo G. Fontana Dalmasso (MGFD) — portfolio web"
                  className={cn(
                    'w-auto invert brightness-0 contrast-200',
                    embedded ? 'h-8' : 'h-10',
                  )}
                />
              </div>
              <div className="flex justify-center gap-5 md:justify-start md:gap-6">
                <a
                  href="https://instagram.com/xmgfd"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-neutral-400 transition-colors hover:text-white"
                  aria-label="Instagram"
                >
                  <svg
                    className={cn(embedded ? 'h-5 w-5' : 'h-6 w-6')}
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                  </svg>
                </a>
                <a
                  href="https://linkedin.com/in/mateofontanad"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-neutral-400 transition-colors hover:text-white"
                  aria-label="LinkedIn"
                >
                  <svg
                    className={cn(embedded ? 'h-5 w-5' : 'h-6 w-6')}
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
                  </svg>
                </a>
                <a
                  href="https://twitter.com/mgfd_"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-neutral-400 transition-colors hover:text-white"
                  aria-label="X"
                >
                  <svg
                    className={cn(embedded ? 'h-5 w-5' : 'h-6 w-6')}
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                  </svg>
                </a>
              </div>
            </div>

            <div className="max-w-full whitespace-nowrap text-center text-[0.6rem] leading-tight tracking-tight text-neutral-400 min-[400px]:text-[0.65rem] sm:whitespace-normal sm:text-xs sm:tracking-normal md:text-right md:text-sm">
              {t('footer.copyright')}
            </div>
          </div>
        )}
      </div>
    </footer>
  );
}
